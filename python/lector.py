...
""" if len(sys.argv) < 2:
    print("❌ Uso: python lector_con_nombre.py usuario")
    sys.exit(1) """

USER_ID = sys.argv[1].replace("@", "") # Corregido: sys.argv[1] para acceder al string
speech_queue = queue.Queue()
is_running = True

def cleanup():
    """Borra mp3 residuales con el prefijo de esta versión"""
    for f in glob.glob("temp_n_*.mp3"):
        try: os.remove(f)
        except: pass
...
def voice_worker():
    ...
    while is_running:
        try:
            ...
            archivo = f"temp_n_{int(time.time() * 1000)}.mp3"
...
async def on_connect(event): print(f"✅ Conectado (Nombres+Mensajes): {event.unique_id}")

@client.on(DisconnectEvent)
async def on_disconnect(event):
    global is_running
    print(f"\n⚠︝ El Live de {USER_ID} terminó. Limpiando..."); is_running = False
    cleanup(); os._exit(0)
...
async def on_comment(event):
    ...
    lectura = f"{event.user.nickname} dice {event.comment}"
    print(f"🎤 {lectura}"); speech_queue.put(lectura)
